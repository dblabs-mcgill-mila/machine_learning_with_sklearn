...
# source
# https://pics.me.me/thumb_very-nice-great-success-great-succes-borat-very-nice-52518578.png
def success():
    success_path = os.path.join(image_folder, 'great_success.jpg')
    display(Image(filename=success_path, width=220, height=180))


# source
# https://sayingimages.com/wp-content/uploads/feeling-depress-fail-meme.jpg
def failed():
    fail_path = os.path.join(image_folder, 'fail.jpeg')
    display(Image(filename=fail_path, width=100, height=200))
...
def _check_boundary_response_method(estimator, response_method):
    ...
    if response_method not in possible_response_methods:
        raise ValueError(
            f"response_method must be one"
            f" of {', '.join(possible_response_methods)}"
        )
